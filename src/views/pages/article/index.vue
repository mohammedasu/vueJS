          ...
          <div class="card-body">
                ...
                <b-card header-class="bg-transparent border-primary"
                  ...
                  <template v-slot:header>
                    <div class="d-flex justify-content-between align-items-center">
                      ...
                      <a href="/sample_article.xlsx" download class="btn btn-primary btn-sm">
                        <i class="fa fa-download mr-1"></i> Download Sample File
                      </a>
                    </div>
                  </template>
                  <h5 class="card-title mt-0">Excel File:</h5>
                  <b-form-group class="mt-2" id="input-group-1"
                    label="Upload Excel File Here [Upload only .csv and .xlsx]" label-for="input-1">
                    <b-form-file
                      accept=".csv, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel"
                      id="input-1" placeholder="Choose a file or drop it here..." ref="file"></b-form-file>
                  </b-form-group>
                  <b-button type="submit" variant="success" class="mr-2" @click.prevent="fetchExcel">Upload</b-button>
                </b-card>
              </div>
            </div>
                ...
                <div class="form-inline navbar-search" v-if="activeTab == 'all'">
                  <div class="input-group">
                    ...
                    <div class="input-group-append">
                      <button id="myBtn" class="btn btn-search">
                ...
                <div class="all-tabs">
                  ...
                  <template v-if="can('add-article')">
                    ...
                    <div class="d-flex align-items-center">
                      ...
                      <router-link :to="{ name: 'add-article' }" class="opt">Add Article</router-link>
                    </div>
                  </template>
                </div>
              </div>
            ...
            <div class="table-responsive mb-0">
              <b-table striped bordered :items="tableData.data" :fields="fields" :sort-by.sync="sortBy" :per-page="0"
                ...
                <!-- <template v-slot:cell(small_description)="row">
                  <p class="small_desc">{{ row.item.small_description }}</p>
                </template> -->
                <template v-can="'update-article-status'" v-slot:cell(is_active)="row">
                ...
                <template v-if="fields[fields.length - 1].key == 'delete'" v-slot:cell(delete)="row"
                  ...
                  <div v-if="activeTab != 'trash'" @click.prevent="deleteArticle(row.item.id)">
                    <i class="mdi mdi-delete delete-i"></i>
                  </div>
                </template>
                <template v-else v-slot:cell(restore)="row" v-can="'restore-article'">
                  <div v-if="activeTab == 'trash'" @click.prevent="restoreArticle(row.item.id)">
