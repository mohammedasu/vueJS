          ...
          <div class="card-body">
                ...
                <div class="all-tabs">
                  ...
                  <template  v-if="can('add-forum')">
                    ...
                    >
                      ...
                      <router-link :to="{ name: 'add-forum' }" class="opt"
                        >Add Forum</router-link
                      >
                    </div>
                  </template>
                </div>
            ...
            <div class="table-responsive mb-0">
              <b-table
                ...
                :key="key"
              >
                <template v-slot:cell(forum_visibility)="row">
                  <template v-if="row.item.forum_visibility == 'public'">
                    <i class="mdi mdi-eye is_visible" v-b-tooltip.hover title="Public Visibility" ></i>
                  </template>
                  <template v-else>
                    <i class="mdi mdi-eye-off is_visible" v-b-tooltip.hover title="Private Visibility" ></i>
                  </template>
                </template>
                <template v-slot:cell(thumbnail_image)="row">
                  <img 
                    :src="row.item.thumbnail_image"
                    alt=""
                    height="50px"
                    width="100px"
                ...
                <template v-can="'edit-forum'" v-slot:cell(edit)="row">
                  <router-link
                    :to="{ name: 'edit-forum', params: { id: row.item.id } }"
                  >
                ...
                <!-- <template v-slot:cell(delete)="row">
                  <div
                    v-if="activeTab == 'trash' && can('restore-forum')"
                    @click.prevent="restoreForum(row.item.id)"
                  >
                    <i class="mdi mdi-restore restore-i"></i>
                  </div>
                  <div
                    v-else-if="activeTab != 'trash' && can('delete-forum')"
                    @click.prevent="deleteForum(row.item.id)"
                  >
